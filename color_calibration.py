...
def sample_color_from_video(video_path, max_samples=3, color_name="Color"):
    ...
    cv2.resizeWindow(window_name, 1280, 720)  # You can adjust these numbers
    # Add speed control (0.5 means half speed, 2.0 means double speed)
    playback_speed = 0.3  # Play at 30% speed
    delay = int(100/playback_speed)  # Increased delay between frames
    ...
    while True:
        ...
        # Add speed controls
        elif key == ord('d'):  # Press 'd' to slow down
            playback_speed = max(0.1, playback_speed - 0.1)
            delay = int(100/playback_speed)
            print(f"Speed: {playback_speed:.1f}x")
        elif key == ord('s'):  # Press 's' to speed up
...
if __name__ == "__main__":
    # Example hardcoded path—modify as needed
    video_path = "video_file_location.mp4"

    # Optional: you can make max_samples a parameter or keep it fixed
    max_samples = 3

    # Run calibration for both blue and yellow
    try:
        calibrate_colors(video_path, max_samples=max_samples)
